...
// MasterChef is the master of CHK. He can make CHK and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once CHK is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of CHK
    ...
    // total naitive tokens each user has harvested
    ...
    constructor(
        ...
        uint256 _startBlock,
        address[] memory _prevMcAddrs,
        uint256[] memory _prevMcMinHarvest
    ) public {
        require(
        ...
        prevMcAddrs = _prevMcAddrs;
        prevMcMinHarvest = _prevMcMinHarvest;
    }

    /**
     *  Returns the amount of tokens harvested by a user.
     */
    function getUserTotalHarvested(address _user)
        public
        view
        returns (uint256)
    {
    ...
    // Update the given pool's CHK allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint256 _depositFeeBP,
    ...
    function pendingCHK(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
    ...
    function massUpdatePools() public {
        ...
        require(length < 50, "massUpdatePools: too many pools");
    ...
    // View function to see user's deposit fee rate
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                harvested[msg.sender] = harvested[msg.sender].add(
                    safeCHKTransfer(msg.sender, pending)
                );
            }
        }
        if (_amount > 0) {
    ...
    function safeCHKTransfer(address _to, uint256 _amount)
        internal
        returns (uint256)
    {
        ...
        if (_amount > chkBal) {
            transferSuccess = chk.transfer(_to, chkBal);
            require(transferSuccess, "safeCHKTransfer: transfer failed");
            return chkBal;
        } else {
            transferSuccess = chk.transfer(_to, _amount);
            require(transferSuccess, "safeCHKTransfer: transfer failed");
            return _amount;
        }
    }
